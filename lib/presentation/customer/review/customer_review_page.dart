...
class _CustomerReviewPageState extends State<CustomerReviewPage> {
  ...
  void _submitForm(EngineerInfo engineerInfo) {
    if(engineerInfo.rating == 0){
      CustomSnackbar.showErrorMessage(context, 'Please do Rating for ${engineerInfo.name} first');
      return;
    }
    CustomSnackbar.showSuccessMessage(context,'Rating submitted for ${engineerInfo.name}: ${engineerInfo.rating}');
    setState(() {
